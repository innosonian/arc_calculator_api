...
후속 사용자 “이메일 도착” 회신으로 직전 안내한 CloudWatch→SNS 시험 이메일 수신을 확인했다. **경보8개 설치·설정 검증과 이번 시험 알림 전달은 통과**로 기록한다. 이메일 원문·Action 이력 출력이나 시험 후 현재 경보 상태를 직접 확인한 것은 아니다. 재발송은 요구하지 않으며 다음 읽기 조회로 현재 State/ActionsEnabled를 확인한다. 실제 장애의 지표 탐지·전체 경보 개별 수신·CloudWatch 로그 수집·전체 AWS 인수 완료로 확대하지 않는다. 문서만 갱신했으며 AWS 호출·설정 변경·메일 발송은 수행하지 않았다.

후속 사용자 `describe-alarms` 출력에서 정확한 프로젝트 경보8개 모두 State=OK/Enabled=true를 확인했다. 앞선 `State:State` 안내 오타로 표시된 null은 잘못된 JMESPath 필드 선택이며, 사용자가 이어 실행한 `State:StateValue` 결과로 현재 상태를 확인했다. 경보 설치·시험메일 수신·시험 후 경보 상태 확인을 마쳤으며 반복 발송이나 강제 상태 변경은 요구하지 않는다. 실제 오류 탐지·로그 수집·전체 AWS 인수 통과로 확대하지 않는다.

다음 단계는 기본 로그 그룹3개의 최근 스트림 메타데이터만 읽는 사용자 실행 안내다. 별도 AI가 CLI 페이지 옵션과 시각 지연·판정 한계를 독립 검토했다. 각 그룹1개, 이름/최근 이벤트/최근 수집 시각만 출력하고 오류 시 나머지 조회를 중단한다. 로그 본문/민감정보 출력·함수 호출·기존 자료 삭제·보관기간 변경은 없으며, 현재 사용자 출력만으로 CloudWatch 실제 로그 수집을 완료 판정하지 않는다.

2026-09-24 후속 사용자 출력에서 API/Worker/Relay 각각의 스트림과 이벤트·수집 시각을 확인했다. 한국 시간의 마지막 수집 시각은 API 09-23 17:53:54.477, Worker 09-23 16:10:13.095, Relay 09-24 06:58:55.137이다. **세 함수의 CloudWatch 로그 수집 흔적 확인은 통과**다. API/Worker의 이전 시험 기록과 Relay의 다음 날 기록은 현재 사용 흐름과 모순되지 않는다. 시각 메타데이터만으로 모든 호출 성공·주기별 실행·로그 무누락·본문 정제를 증명하지 않으며 전체 AWS 인수는 계속 진행 중이다. 별도 AI 검토도 이 판정 범위를 확인했다. 에이전트의 AWS 조회·실행·설정 변경은 없었다.

다음 세션 간 조회 차단 시험용 `api-ownership-check.py`를 비공개/Git 제외 위치에 준비했다(SHA256 `653a8a584162a69307103485e8a45115d0b85d1d6a6a192f8d01a3d90e0b08bd`). 별도 담당이 기존 helper·완료 체크포인트 읽기, 정상 세션/결과 대조와 새 로그인1회/유효성 확인/세 경로404 검사를 구현하고 계약을 검토했다. 주 검토자는 차트 경로 자체가 고장 나도 차단 성공으로 오인하지 않도록 원래 세션의 차트 발급200도 선행 확인하도록 보완했다. 총10요청 중 쓰기는 Dummy 로그인1회이며 훈련·진도·체크포인트·AWS 설정은 변경하지 않는다.

최종 도구의 합성 HTTPS 응답·임시 체크포인트를 이용한 실제 main 오프라인 시험15개가 통과했다. 정상 경로, 만료된 원래 세션, 새 세션 인증 실패, 다른 세션의200/403/다른404 오류, 원래 결과 변경·차트 경로 부재, 중복 JSON·잘못된 헤더·리다이렉트·시간 초과·큰 응답·같은 로그인 세션·체크포인트 연결 불일치를 확인했다. 모든 사례에서 자동 재시도 없이 최대 로그인1회, 민감값 출력 없음과 체크포인트 바이트/권한 보존을 확인했다. 구문·권한0600·Git 제외·문서 공백 검사도 통과했다. 실제 AWS 요청·사용자 체크포인트 접근은0이며 런타임/ZIP은 변경하지 않았다. 이 도구를 통한 실제 AWS 소유권 시험은 사용자 후속 실행 대상이다.

2026-09-24 후속 사용자 `api-ownership-check.py` 출력은 원래 세션·훈련·성공 결과·차트 발급200, 별도 로그인201·새 세션 유효성200, 다른 세션의 훈련/계산/차트 발급 각각404 NOT_FOUND, 원래 결과200/SameResult=true 및 OriginalCheckpointPreserved=true/ALL_PASS였다. **이 기존 일반 훈련에 대한 같은 계정의 세션 간 조회 차단은 실제 AWS 사용자 출력 범위에서 통과**다. 원래 세션의 정상 조회와 다른 세션의 유효성을 먼저 확인했으므로 만료된 토큰이나 없는 경로의404를 소유권 차단으로 오인한 판정이 아니다. 새 세션의 개인 식별·다른 계정/환경 격리·재인가·시간 경과 만료·중복 처리까지 확장하지 않는다. 사용자 체크포인트·토큰·결과 원문은 공유되지 않았고 에이전트의 AWS 호출도 없다.

2026-09-24 사용자는 FitCloud 비용 조회 계정을 아직 받지 못했다고 확인했다. 비용 조회·예산 알림 검증은 계정 전달 이후 진행할 외부 준비 항목이며, 현재 기술 시험의 성공을 비용 통제 완료로 확대하지 않는다.

다음 실제 차트 만료/재발급 확인용 `api-chart-expiry-check.py`를 준비했다(SHA256 `470f85c19ad96707627f82190aff8c1fffce9659efd20fff0f8622f3644b9392`, 권한0600·Git 제외). 별도 담당의 소스 검토는 차트 발급이 기존 완료 작업·검증된 비공개 snapshot을 읽고300초 링크를 서명하며 훈련·진도를 변경하지 않음을 확인했다. 공식 AWS의 시간 경과 후 AccessDenied/Request has expired와 임시 자격증명 ExpiredToken을 구별한다. 기존 체크포인트와 고정 체크섬 helper2개를 읽고, 원래 세션/결과·새 링크 다운로드200→실제 만료까지 대기→같은 링크의 만료403→재발급200·같은 객체/본문·원래 결과 보존을 검사하도록 구현했다. 총8GET이며 모든 토큰·URL·본문은 메모리에만 보관하고 로그인·POST·CLI·재시도·재인가는 없다.

주 검토자의 실제 main 오프라인 시험23개가 통과했다. 합성 시계·HTTPS 응답·임시 체크포인트로 global/regional 정상 흐름, 세션 만료/여유 부족, 일반403/ExpiredToken/만료 후200, 재발급 객체·본문/원래 결과 변경, 시계 변동/정지, 외부 host·잘못된TTL·리다이렉트·통신 시간 초과·잘못된 헤더·중복JSON·다른 차트·DOCTYPE/중복Code/큰 응답/UTF16 XML 거절을 확인했다. 차트 본문은 합성 데이터와 메모리의 시험용 기대 hash를 사용했으며 실제 기존 fixture 계산이나 AWS 시간 경과 시험을 대신하지 않는다. 모든 사례에서 GET 한정·S3로 Bearer 미전송·민감값 출력 없음·체크포인트 바이트/권한 보존, 정상 흐름8GET·동일 URL 재사용·한 번의 sleep30초 이하를 확인했다. 실제 대기는 가상 시계로 진행했고 네트워크·사용자 체크포인트 접근은0이다. 구문·두 원본 helper 불변·Git 제외·문서 공백 검사도 통과했다. 런타임/ZIP 변경은 없으며 실제 AWS300초 만료/재발급 결과는 사용자 후속 실행 대상이다.

2026-09-24 후속 사용자 `api-chart-expiry-check.py` 출력은 세션 시간 여유 충족, global 주소의 새300초 차트 링크·다운로드200, WAITING307초부터7초까지의 대기, 같은 링크의403 AccessDenied/ExpiryConfirmed=true, 재발급 링크·같은 fixture 차트200, 원래 결과200/SameResult=true 및 OriginalCheckpointPreserved=true/ALL_PASS였다. **이번 기존 일반 훈련 차트의 실제 시간 경과 만료 거절·재발급·동일 본문 보존은 사용자 실행 AWS 출력 범위에서 통과**다. 설정값300초만 확인한 이전 검증을 넘어 같은 링크의 실제 만료 오류와 재발급 정상 조회를 대조했다. 서명 URL·토큰·본문은 공유되지 않았고 에이전트의 실제 AWS 호출은 없다. 세션24시간 만료·재인가·재전송·장애 시험이나 전체 AWS 인수로 확대하지 않는다.

다음 완료 요청의 순차 재전송용 `api-replay-check.py`를 준비했다(SHA256 `c0239334297a6790fc8c539f0141207cbe6713abcac495eab43d3265965a0af7`, 권한0600·Git 제외). 별도 담당과 주 검토자가 실제 course의 COURSE_CREATE 접수 행/typed request digest·생성 당시 receipt200 및 같은 입력 digest의 저장 결과 반환 경로를 대조했다. 도구는 기존 helper2개/fixture/체크포인트, 유효 세션·완료 API 결과, 정확한 계정의 receipt/attempt/done job3행을 검증한 뒤 기존 시작 본문과 같은 파일/조건을 각1회 POST한다. 이후 같은3행 전체·API 훈련/결과·체크포인트 불변을 확인한다. 운용 로그는 생길 수 있으며 DB 전체 무변경·동시 편집 방지·SQS 중복 전달 인수로 확대하지 않는다.

최종 도구의 실제 main을 실행한 오프라인 시험22개가 통과했다. 실제 기존 fixture와 합성 체크포인트·HTTP·CLI 응답으로 정상 HTTP7회/STS1회/GetItem6회, 같은 clientRequestId·본문·바이너리/조건 전송, 입력/계정/권한/세션/접수 행/작업/조건 불일치의 POST 전 중단,201/202·다른 receipt/결과·두 POST 각각의 시간 초과·저장 행/API 결과 변경·리다이렉트·중복JSON·잘못된 헤더·큰 응답 거절을 확인했다. 모든 사례에서 POST 최대2회·자동 재시도 없음·DDB 강한 읽기·CLI 행 키 파일0600/상위0700·민감값 출력 없음·원본 체크포인트 바이트/권한 보존을 확인했다. 마지막 정보 필드 이름은 API POST와 혼동하지 않도록 AWSCLIReadOnly로 명확히 했다. 원본 helper/fixture·런타임/ZIP은 변경하지 않았고 실제 AWS/네트워크/사용자 체크포인트 접근은0이다. 구문·원본 helper hash·Git 제외·문서 공백 검사도 통과했으며 실제 AWS 재전송 시험은 사용자 후속 실행 대상이다.

2026-09-24 후속 사용자 `api-replay-check.py` 출력은 기존 성공 결과와 입력 digest/완료 작업/원래 접수 기록 일치, 시작 재전송200/SameAttempt·SameReceipt=true, 같은31380bytes 파일 재전송200/SameCompletedResult=true, 훈련·작업·접수 기록·결과 각각 Unchanged=true와 OriginalCheckpointPreserved=true/ALL_PASS였다. **이 완료된 일반 훈련의 순차 재전송은 실제 AWS 사용자 출력 범위에서 통과**다. POST는2회였고 AWS CLI는 읽기만 수행했다. 새 훈련을 만들거나 기존 결과를 덮어쓰지 않았다는 검증 범위이며, DB 전체 무변경·동시/처리 중 요청·SQS 중복 전달·장애 복구 성공을 뜻하지 않는다. 에이전트는 이번 결과를 문서에 반영했으며 AWS 호출·런타임/ZIP 변경은 하지 않았다.

현재 판정은 **공유 Dummy Dev의 검증된 기본 흐름 통과·앱 연결 시험 가능**이다. HTTPS 로그인/조회부터 기존 성인 압박 Only 자료의 훈련/최종평가·저장·차트·조회 소유권·차트 만료·완료 요청 재전송까지의 증거에 한정한다. 실제 앱/마네킨 자료와 다른 훈련 종류·최대 크기/동시성, 세션 만료/복구·작업 중복/장애 복구의 실제 AWS 시험은 남아 있다. FitCloud 계정 전달 전 비용 조회·예산 알림도 미확인이다. ARC 공식 연동은 계속 비활성이며 이 판정을 Beta/Prod·전체 운영 승인으로 확대하지 않는다. 별도 AI의 문서 읽기 검토도 제한된 앱 연결·조회 시험을 막는 추가 결함을 찾지 못했으며, 전체 운영 인수와 구별하는 판정에 일치했다. 다음 사용자 단계는 [앱팀 연결 안내](DEPLOY_GUIDE.md#beginner-dev-app-handoff)다. 이번 변경은 문서의 실제 시험 결과·전달 안내 반영뿐이므로 런타임 회귀나 AWS 시험을 반복하지 않았다.
